        ...
        for epoch in range(self.epochs):
            ...
            for (nbatch, batch) in enumerate(temp_batchqueue):
                ...
                if not nbatch % self.logging:
                    ...
                    out += self.__get_result(comment_batch, 
                                             target_comment,
                                             batch.code_oovs)
                    if log_to_file:
                        logfile.write(f"{out}\n")
            ...
            print(f"Epoch completed in {(time() - start)/60.0:.2f} min")
    ...
    def __get_result(self, comment_batch, target_comment, oovs, print_idx=None):
        ...
        for i in range(batch_len):
            ...
            if print_idx is not None and print_idx == i:
                ...
                print(f"Ground Truth: {' '.join(target)}")
...
class Tester:
    ...
    def __test_step(self, batch):
        ...
        comment_batch, coverage = [], None
        for i in range(1, batch.nl.shape[1]):
            final_dist, _, state, coverage = self.decoder(dec_input, hidden,
                                                          prev_state=state,
                                                          coverage=coverage,
                                                          code_ext=batch.code_ex,
                                                          max_oovs=batch.max_code_oovs)
        ...
        comment_batch = np.array(comment_batch).T
        return comment_batch, batch.nl[:, 1:]
    ...
    def test(self, log_to_file=False):
        ...
        for batch in temp_batchqueue:
            comment_batch, target_comment = self.__test_step(batch)
            out = self.__get_result(comment_batch, 
                                    target_comment, 
                                    batch.code_oovs,
                                    print_idx=10)
